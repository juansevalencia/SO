...
int main(int argc, char const* argv[]) {
  ...
  int pipe_resultado[2]; //PORQUE NO CREA UN PIPE RESULTADO PARA CADA PROCESO?
  pipe(pipe_resultado); //como no se pisan entre si los resultados?
  ...
  for (int i = 0; i < procesos; i++) {
    ...
    write(pipes[i][WRITE], &desde, sizeof(desde)); //como no se pisan? //no habrai que crear dos pipes
    write(pipes[i][WRITE], &hasta, sizeof(hasta)); //si el scheduler le pasa  aotro no habria q cerrar el pipe?
