...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	//En que block group esta? numero de inodo / cant inodos x block group
	unsigned int numero_de_grupo_de_bloque = blockgroup_for_inode(inode_number);
	
	//donde esta la table->te lo dice el descriptor
	Ext2FSBlockGroupDescriptor* descriptor_de_grupo_de_bloque_i = block_group(numero_de_grupo_de_bloque); 

	unsigned int tabla_de_inodos = descriptor_de_grupo_de_bloque_i->inode_table;
	

	//1.1*necesito saber en que conjunto de inodos esta-> en que bloque esta mi conjunto de inodos en la table
	
	//para eso necesito saber cuantos inodos hay por bloque
	unsigned int cant_inodos_x_bloque = block_size / (superblock()->inode_size);

	//inodo i esimo en mi inode table / cant inodos x bloque
	unsigned int numero_de_bloque_inodetable = blockgroup_inode_index(inode_number) / cant_inodos_x_bloque;


	//quiero devolver el puntero al inodo.
	unsigned char* res = (unsigned char*)malloc(superblock()->inode_size); //inicializo el nodo que voy a devolver con tamaño indoo
	//reservo memoria para traerme el bloque . No puedo leer directamente el inodo porque se lee de a bloques no?
	unsigned char* bloque = (unsigned char*)malloc(block_size);

	//ACA DIFIERO DE EZE, se que mi tabla de inodos tiene m bloques de tamaño block size, no deberia moverme de a block size?
	read_block(tabla_de_inodos + (numero_de_bloque_inodetable << block_size), bloque);

	unsigned int posicion_en_bloque = (blockgroup_inode_index(inode_number)) % cant_inodos_x_bloque ; //aca bloque es mi bloque dentro de mi tabla de inodos

	unsigned int k = (posicion_en_bloque * (superblock()->inode_size));
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	}else if(block_number < 12 + 256 ){  // entran 1024/4  direcciones
		//es indirecto
		//unsigned int* bloque_indirecto = (unsigned int*)inode->block[12]; //puntero a bloque indirecto 
		//return bloque_indirecto[block_number - 12]; // puedo no tener esa memoria asinada a mi proceso o si serlo y leer cualquier cosa

		unsigned char* bloque_indirecto = (unsigned char*)malloc(1024<< superblock()->log_block_size);

		read_block(inode->block[12], bloque_indirecto);

		unsigned int res = ((unsigned int*)bloque_indirecto)[(block_number - 12)];

		delete[] bloque_indirecto;

		return res;

	}else{
	//indirecto doble 
		unsigned char* bloque_indirecto_doble = (unsigned char*)malloc(1024<< superblock()->log_block_size); //reservo para el bloque indirecto doble

		read_block(inode->block[13], bloque_indirecto_doble); // me traigo el contenido al bloque indirecto doble
		
		unsigned char* bloque_indirecto_ = (unsigned char*)malloc(1024<< superblock()->log_block_size); //reservo para un bloque indirecto

		int k = (block_number - 12 - 256) / 256 ;

		read_block(bloque_indirecto_doble[k], bloque_indirecto_); // 
		
		unsigned int res = ((unsigned int*)bloque_indirecto_)[((block_number - 12 - 256) % 256)];

		delete[] bloque_indirecto_;

		delete[] bloque_indirecto_doble;
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	//queremos devolver el inodo donde esta el archivo con nombre filename, from es desde donde aprtimos


	//recorrer directorios hasta encontrar donde esta mi archivo.
	int largo = strlen(filename);

	unsigned char* res = (unsigned char*)malloc(superblock()->inode_size);
	
	

	Ext2FSDirEntry entrada = ;
	if(strlen(entrada.name_length == largo && strcmp(filename, entrada.name) == 0 ) ){
		read_block( ,res); //lee de entrada. 
	}
